...
export function parseAstJsonNative(code) {
    const exe = findTsparser();
    const result = childProcess.spawnSync(exe, [], { input: code, encoding: 'utf-8' });
    if (result.status !== 0) {
...
function findTsparser() {
    const exeName = platformIsWindows() ? 'tsparser.exe' : 'tsparser';
    const envPath = process.env.TYPENATIVE_TSPARSER;
