...
const KIND_NAMES: Record<number, string> = (() => {
  ...
  for (const key of Object.keys(ts.SyntaxKind)) {
    const val = (ts.SyntaxKind as any)[key];
    if (typeof val === 'number' && names[val] === undefined) names[val] = key;
  }
  return names;
})();
...
function convertNode(node: ts.Node): any {
  const obj: any = { kind: KIND_NAMES[node.kind] ?? String(node.kind) };
  if (TEXT_KINDS.has(node.kind)) {
    obj.text = (node as any).text ?? (node as any).escapedText;
  }
...
export function parseAstJson(code: string): any {
